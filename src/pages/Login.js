...
export default function Login() {
  const ref = useRef();
  const [username, setUsername] = useState(""); //state olarak belirlenir
  const [password, setPassword] = useState(""); //state olarak belirle

  const enable = username && password; //eğer böyle bir değişken var ise Login butonunun akitf olup olmadığunu belirlemek için kullanılır
      ...
      <div className="w-[350px] grid gap-y-3">
        <div className="bg-white border px-[40px] pt-10 pb-6">
          ...
          <form className="grid gap-y-1.5">
            ...
            <button
              type="submit"
              disabled={!enable} //yukarıda enable false olduğu için disabled değerini true yapıldı.
